...
def timetable(request, class_id):
    ...
    matrix = [['' for i in range(9)] for j in range(5)]
...
def t_timetable(request, teacher_id):
    ...
    class_matrix = [[True for i in range(9)] for j in range(5)]
    for i, d in enumerate(DAYS_OF_WEEK):
        t = 0
        for j in range(9):
            if j == 0:
...
def marks_list(request, stud_id):
    ...
    for ass in ass_list:
        try:
            ...
            sc.save()
            sc.marks_set.create(type='C', name='Class test 1')
            sc.marks_set.create(type='C', name='Class test 2')
